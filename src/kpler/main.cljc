(ns kpler.main
  "Kotodama WASM entrypoint for the Kpler clean-room actor (L4) — Clojure port.
  ...
  facts (`kpler.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"kpler.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kpler-compat" :tier tier :entities entities} 200])
